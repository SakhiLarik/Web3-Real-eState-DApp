...
const PropertyRequests = () => {
  ...
  const approveRequest = async (requestId) => {
    ...
    setSuccess('');
    try {
      const response = await axios.post(`${api}/admin/property/approve/${requestId}`, {
        wallet: auth.user.wallet,
      });
      if (response.status === 200) {
        setSuccess(`Request ${requestId} approved and minted! Token ID: ${response.data.tokenId}`);
        // Refresh requests
        const res = await axios.get(`${api}/admin/property/requests?wallet=${auth.user.wallet}`);
        setRequests(res.data.requests);
      } else {
  ...
  const rejectRequest = async (requestId) => {
    ...
    setSuccess('');
    try {
      const response = await axios.post(`${api}/admin/property/reject/${requestId}`, {
        wallet: auth.user.wallet,
      });
      if (response.status === 200) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {requests.map(req => (
                <div key={req._id} className="border p-4 rounded shadow">
                  ...
                  <p><strong>User:</strong> {mediumAddress(req.userAddress)}</p>
